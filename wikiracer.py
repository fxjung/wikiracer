...
def highlight_disabled_links() -> bool:
    """Return whether disabled links should be rendered in bold red text."""
...
def main() -> None:
    ...
    while index < len(args):
        arg = args[index]
        if arg in ("--highlight-disabled-links", "--red-disabled-links"):
            environ["WIKIRACE_HIGHLIGHT_DISABLED_LINKS"] = "1"
            index += 1
            continue
        if arg.startswith(("--highlight-disabled-links=", "--red-disabled-links=")):
            enabled = arg.split("=", 1)[1].strip().lower()
    ...
    sys.argv = ["mitmproxy", "-s", str(Path(__file__).resolve()), *filtered_args]
    mitmproxy()
